        ...
        <table class="min-w-full divide-y divide-slate-200">
            <thead class="bg-slate-50 text-left text-xs font-semibold uppercase tracking-wide text-slate-500">
                <tr>
                    ...
                    <th class="px-4 py-3">Référent</th>
                ...
                @forelse ($patients as $patient)
                    <tr class="hover:bg-slate-50">
                        <td class="px-4 py-3 text-sm">
                            <div class="font-semibold text-slate-700">{{ $patient->reference_code }}</div>
                            <div class="font-mono text-xs text-slate-400">{{ $patient->external_id }}</div>
                        </td>
                        <td class="px-4 py-3 font-medium text-slate-700">{{ $patient->full_name }}</td>
                        <td class="px-4 py-3 text-slate-500">{{ $patient->primaryAgent->name ?? 'Non attribué' }}</td>
                        <td class="px-4 py-3 text-slate-500">{{ $patient->screenings_count }} dépistage(s)</td>
