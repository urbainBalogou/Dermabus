...
@section('content')
    ...
    <form method="GET" class="mb-6 grid gap-4 md:grid-cols-4">
        ...
        <div>
            <label class="text-sm font-medium text-slate-600" for="treatment_status">Suivi traitement</label>
            <select name="treatment_status" id="treatment_status" class="mt-1 w-full rounded-md border-slate-300 focus:border-emerald-500 focus:ring-emerald-500">
                <option value="">Tous</option>
                <option value="pending" @selected(request('treatment_status') === 'pending')>À initier</option>
                <option value="in_progress" @selected(request('treatment_status') === 'in_progress')>En cours</option>
                <option value="completed" @selected(request('treatment_status') === 'completed')>Terminé</option>
                <option value="not_required" @selected(request('treatment_status') === 'not_required')>Non requis</option>
            </select>
        </div>
            ...
            <tbody class="divide-y divide-slate-200 text-sm">
                @forelse ($screenings as $screening)
                    <tr class="hover:bg-slate-50">
                        ...
                        <td class="px-4 py-3 text-slate-500">{{ Str::headline($screening->treatment_status) }}</td>
